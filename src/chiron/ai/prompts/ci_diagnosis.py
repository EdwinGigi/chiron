...
A GitHub Actions workflow has failed. Your task is to diagnose the root cause of the failure from
the provided logs and map it to specific files and line numbers if possible.
...
Output a `CIDiagnosisResult` containing an explanation of the failure, and a list of
`ReviewComment` objects. For each comment, specify the EXACT file path and line number
where the fix needs to be applied, and describe what the fix should be.
If the log does not clearly indicate a specific file/line (e.g., a network timeout or
infrastructure failure), return an empty list of comments.
"""
